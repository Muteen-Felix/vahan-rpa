...
2. Khi alarm chạy, extension mở VAHAN Public Report trong tab nền.
...
Health check không điền filter, không đọc/giải CAPTCHA, không bấm Apply và
không tải báo cáo VAHAN.
...
- `health-check.mjs`: alarm, tab nền, so sánh dữ liệu, gửi log backend, queue
  log offline, queue alert và state.
